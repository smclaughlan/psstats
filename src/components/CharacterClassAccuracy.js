    ...
    stats.forEach((stat) => {
      if (stat.stat_name === "hit_count") {
        ...
        stats.forEach((fireStat) => {
          if (
            fireStat.stat_name === "fire_count" &&
            fireStat.profile_id === profileNum
          ) {
            chartData.push({
              name: stat.class.name.en,
              percent:
                (Number.parseInt(stat.value_forever) /
                  Number.parseInt(fireStat.value_forever)) *
                100,
              percentStr: `${Math.floor(
                (Number.parseInt(stat.value_forever) /
                  Number.parseInt(fireStat.value_forever)) *
                  100
              )}%`,
